...
class Logger(object):
    ...
    def __init__(self, base_dir="../APP_LOG/", level='info',
        ...
        debug_file_handle = ConcurrentRotatingFileHandler(f"{log_dir_dic['log_debug_dir']}{date_now}.log",
                                                          encoding='utf-8')
        debug_file_handle.setLevel("DEBUG")  # 设置输出的日志等级为DEBUG
        info_file_handle = ConcurrentRotatingFileHandler(f"{log_dir_dic['log_info_dir']}{date_now}.log",
                                                         encoding='utf-8')
        info_file_handle.setLevel("INFO")  # 设置输出的日志等级为INFO以上
        warning_file_handle = ConcurrentRotatingFileHandler(f"{log_dir_dic['log_warning_dir']}{date_now}.log",
                                                            encoding='utf-8')
        warning_file_handle.setLevel("WARNING")  # 设置输出的日志等级为WARNING以上
        error_file_handle = ConcurrentRotatingFileHandler(f"{log_dir_dic['log_error_dir']}{date_now}.log",
                                                          encoding='utf8')
